  ...
  <div class="auth-container">
    <div class="auth-card glass-panel">
      ...
      <div class="social-login">
        <button @click="handleGoogleLogin" class="btn btn-secondary btn-block btn-google" :disabled="loading">
      ...
      <form @submit.prevent="handleSubmit" class="auth-form">
        ...
        <!-- Submit Button -->
        <button type="submit" class="btn btn-primary btn-block btn-auth" :disabled="loading">
          <span v-if="loading" class="spinner"></span>
          <span v-else>{{ activeTab === 'signin' ? 'Se connecter' : 'Créer un compte' }}</span>
        </button>
      </form>
...
const handleSubmit = async () => {
  ...
  try {
    if (activeTab.value === 'signin') {
      ...
      if (error) throw error
      successMessage.value = "Compte créé ! Veuillez vérifier vos e-mails pour valider votre inscription (ou connectez-vous si la confirmation mail est désactivée dans Supabase)."
    }
  } catch (err: any) {
    errorMessage.value = err.message || "Une erreur s'est produite lors de l'authentification."
  } finally {
    loading.value = false
  }
}
