...
impl LlmClient {
    pub fn new() -> Self {
        let api_url = env::var("LLM_API_URL").unwrap_or_else(|_| "http://localhost:1337/v1".to_string());
    ...
    pub async fn evaluate_answer(
        ...
        let system_prompt = "Tu es un tuteur de japonais bienveillant pour l'application LearnWithManga. \
...
Ne mets aucun texte explicatif avant ou après le JSON. N'utilise pas de blocs markdown.";
        ...
        if !response.status().is_success() {
            ...
            return Err(format!("Le serveur LLM a répondu avec l'erreur {}: {}", error_status, error_text));
        }
        ...
        let eval: EvaluateResponse = serde_json::from_str(&cleaned_json)
            .map_err(|e| format!("Échec du parsing JSON de la réponse LLM. Reçu: \"{}\". Erreur: {}", raw_content, e))?;
    ...
    pub async fn generate_hint(
        ...
        let system_prompt = "Tu es un tuteur de japonais pour l'application LearnWithManga. \
...
Sois très concis (1-2 phrases maximum) et réponds directement en français.";
    ...
    pub async fn get_models(&self, req: &ModelsRequest) -> Result<Vec<String>, String> {
        ...
        if !response.status().is_success() {
            let error_status = response.status();
            let error_text = response.text().await.unwrap_or_default();
            return Err(format!("Le serveur LLM a répondu avec l'erreur {} lors de la récupération des modèles: {}", error_status, error_text));
        }
